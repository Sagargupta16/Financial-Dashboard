...
export const FinancialHealthScore = ({
  ...
  const healthData = useMemo(() => {
    ...
    const data = {
      income: kpiData.totalIncome || 0,
      expenses: totalExpenses,
      savings: (kpiData.totalIncome || 0) - totalExpenses,
    ...
    <div className="bg-gray-800/50 rounded-2xl p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1">
          <div
            className={`bg-gradient-to-br ${getGradient(score.score)} rounded-xl p-6 text-center`}
          >
            <p className="text-white/80 text-sm mb-2">Overall Score</p>
            <p className={`text-6xl font-bold ${getScoreColor(score.score)}`}>
              {score.score}
            </p>
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-700/50 rounded-lg p-4">
              ...
              <div className="mt-2 h-2 bg-gray-600 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, score.savingsRate * 5)}%` }}
                />
              </div>
            </div>
            ...
            <div className="bg-gray-700/50 rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-white mt-1">
                {score.monthsCovered} months
              </p>
              <div className="mt-2 h-2 bg-gray-600 rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-green-600 to-blue-600"
                  style={{
                    width: `${Math.min(100, (parseFloat(score.monthsCovered) / 6) * 100)}%`,
                  }}
                />
              </div>
            </div>
